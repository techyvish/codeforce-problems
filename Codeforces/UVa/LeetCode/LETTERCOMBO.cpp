...
void combination(string &s , int l , int noOfLoops , int noOfIteration,  map<char,vector<char> > &amap, string &res, vector<string> &result)
{
    if ( l == noOfLoops )
    {
        string k = res;
    ...
    {
        res[l] = amap[s[l]][i];
        combination(s,l+1,noOfLoops,noOfIteration,amap,res,result); // combination
    }
}
...
int main() {
    ...
    combination(str, 0 , str.length() , 4 , amap , res , result);
